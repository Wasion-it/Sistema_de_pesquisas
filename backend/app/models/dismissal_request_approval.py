...
class DismissalRequestApproval(BaseModel):
    __tablename__ = "dismissal_request_approvals"
    __table_args__ = (
        Index("ix_dismissal_request_approvals_dismissal_request_id", "dismissal_request_id"),
        Index("ix_dismissal_request_approvals_status", "status"),
        Index("ix_dismissal_request_approvals_step_order", "step_order"),
    )

    dismissal_request_id: Mapped[int] = mapped_column(
        ForeignKey("dismissal_requests.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    request = relationship("DismissalRequest", back_populates="approval_steps")
